...
const postSale = orders.map(totals => (totals.price * totals.tax) + totals.price)
...
//const bobsTotal = purchases.filter(total => total.owner === "Bob").reduce((sum, curr) => sum.price + curr.price)

const bobsOrders = purchases.filter(total => total.owner === "Bob")

console.log(bobsOrders)

const bobsTotal = bobsOrders.reduce((acc, curr) => 
  {
